...
public class TargetTransformationSource {
	...
	protected void main()
		...
		response.sendRedirect(StringEscapeUtils.escapeEcmaScript(request.getParameter("page")) );

		//Transformation level 1
		response.sendRedirect(transformationLevel1Positiv(request.getParameter("page")) );
		
		//Transformation level 2
		response.sendRedirect(transformationLevel2Positiv(request.getParameter("page")) );
				
		//Transformation level 3
		response.sendRedirect(transformationLevel3Positiv(request.getParameter("page")) );
		
		//Transformation level 1 Negative
		response.sendRedirect(transformationLevel1Negative(request.getParameter("page")) );
	
		//Transformation level 2 Negative
		response.sendRedirect(transformationLevel2Negative(request.getParameter("page")) );
		
		//Transformation level 3 Negative
		response.sendRedirect(transformationLevel3Negative(request.getParameter("page")) );
