...
public class Source_TargetTransformation {
	...
	protected void responseSendError()
    	...
    	response.addHeader("X-Header",  transformationLevel1Positiv(requestParameter));
    	
    	// Transformation level 2 positive
     	response.addHeader("X-Header",  transformationLevel2Positiv(requestParameter));
    	
    	// Transformation level 3 positive
    	response.addHeader("X-Header",  transformationLevel3Positiv(requestParameter));
    	
    	// Transformation level 1 negative
    	response.addHeader("X-Header",  transformationLevel1Negative(requestParameter));
 
    	//Transformation level 2 negative
    	response.addHeader("X-Header",  transformationLevel2Negative(requestParameter));
    	
    	//Transformation level 3 negative
    	response.addHeader("X-Header",  transformationLevel3Negative(requestParameter));
